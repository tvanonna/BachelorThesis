...
$Iterations = 9, 9, 9, 9, 4, 4, 2, 2, 2, 2
...
deno run --allow-all --unstable-raw-imports ../src/main.js 256 256 18 10000000 false false | Out-File -FilePath "benchdata/settings.log"
...
for(($i = 0); $i -lt $S_length; $i++){
    ...
    for(($j = 0); $j -le $iter; $j++){
        ...
        (Measure-Command {deno run --allow-all --unstable-raw-imports ../src/main.js $size $size 18 1000000 true false}).ToString() | Out-File -Append -FilePath "$($base_dir1)/output.log" 
        #coalescing turned off, morton turned off
        (Measure-Command {deno run --allow-all --unstable-raw-imports ../src/main.js $size $size 18 1000000 false false}).ToString() | Out-File -Append -FilePath "$($base_dir2)/output.log" 
        #coalescing turned on, morton turned on
        (Measure-Command {deno run --allow-all --unstable-raw-imports ../src/main.js $size $size 18 1000000 true true}).ToString() | Out-File -Append -FilePath "$($base_dir3)/output.log" 
        #coalescing turned off, morton turned on
        (Measure-Command {deno run --allow-all --unstable-raw-imports ../src/main.js $size $size 18 1000000 false true}).ToString() | Out-File -Append -FilePath "$($base_dir4)/output.log" 
    }
